...
void TFRTEngine::addInput(string layer, nvinfer1::DimsCHW dims, size_t eleSize){
    /*
...
bool TFRTEngine::loadUff(const char* uffFile, size_t maximumBatchSize, nvinfer1::DataType dataType=nvinfer1::DataType::kFLOAT){
...
std::vector<std::vector<void*>> TFRTEngine::predict(std::vector<std::vector<void*>> batchInputs){
    ...
    for(int b=0; b<batchCount; b++){
		...
		for(int i=0; i<networkInputs.size(); i++){
			...
			CHECK(cudaMemcpy(GPU_Buffers[bindingIdx + b*stepSize], batchInputs[b][i], inputSize, cudaMemcpyHostToDevice));
			bindingIdx++;
		}
    ...
    for(int b=0; b<batchCount; b++){
		...
		for(int i=0; i< networkOutputs.size(); i++){
			...
			CHECK(cudaMemcpy(Output_Buffers[b][i], GPU_Buffers[bindingIdx + b*stepSize], outputSize, cudaMemcpyDeviceToHost));
